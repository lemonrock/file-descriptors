...
extern "C"
{
	...
	pub(crate) fn fanotify_mark(fanotify_fd: RawFd, flags: c_uint, mask: uint64_t, dirfd: RawFd, pathname: *const c_char) -> c_int;
}

/// Create an event when a file or directory is accessed (read).
pub(crate) const FAN_ACCESS: uint64_t = 0x01;

/// Create an event when a file is modified (write).
pub(crate) const FAN_MODIFY: uint64_t = 0x02;

/// Create an event when a writable file is closed.
pub(crate) const FAN_CLOSE_WRITE: uint64_t = 0x08;

/// Create an event when a read-only file or directory is closed.
pub(crate) const FAN_CLOSE_NOWRITE: uint64_t = 0x10;

/// Create an event when a file or directory is opened.
pub(crate) const FAN_OPEN: uint64_t = 0x20;

/// Create an event when an overflow of the event queue occurs.
///
/// The size of the event queue is limited to 16384 entries if `FAN_UNLIMITED_QUEUE` is not set in `fanotify_init()`.
pub(crate) const FAN_Q_OVERFLOW: uint64_t = 0x4000;

/// Create an event when a permission to open a file or directory is requested.
///
/// A fanotify file descriptor created with `FAN_CLASS_PRE_CONTENT` or `FAN_CLASS_CONTENT` is required.
pub(crate) const FAN_OPEN_PERM: uint64_t = 0x10000;

/// Create an event when a permission to read a file or directory is requested.
///
/// A fanotify file descriptor created with `FAN_CLASS_PRE_CONTENT` or `FAN_CLASS_CONTENT` is required.
pub(crate) const FAN_ACCESS_PERM: uint64_t = 0x20000;
...
pub(crate) const FAN_ONDIR: uint64_t = 0x40000000;
...
pub(crate) const FAN_EVENT_ON_CHILD: uint64_t = 0x08000000;

/// A file is closed.
///
/// This is the combination of `FAN_CLOSE_WRITE` and `FAN_CLOSE_NOWRITE`.
pub(crate) const FAN_CLOSE: uint64_t = FAN_CLOSE_WRITE | FAN_CLOSE_NOWRITE;

/// This is a combination of `FAN_ACCESS`, `FAN_MODIFY`, `FAN_CLOSE` and `FAN_OPEN`.
pub(crate) const FAN_ALL_EVENTS: uint64_t = FAN_ACCESS | FAN_MODIFY | FAN_CLOSE | FAN_OPEN;

/// This is a combination of `FAN_OPEN_PERM` and `FAN_ACCESS_PERM`.
pub(crate) const FAN_ALL_PERM_EVENTS: uint64_t = FAN_OPEN_PERM | FAN_ACCESS_PERM;

/// This is a combination of `FAN_ALL_EVENTS`, `FAN_ALL_PERM_EVENTS` and `FAN_Q_OVERFLOW`.
pub(crate) const FAN_ALL_OUTGOING_EVENTS: uint64_t = FAN_ALL_EVENTS | FAN_ALL_PERM_EVENTS | FAN_Q_OVERFLOW;
